...
if (this.getElementos() != grafica.getElementos() || this.getAristas() != grafica.getAristas())
    ...
    for (Vertice v : this.vertices) {
        VerticeGrafica<?> verticeComparar = grafica.vertice(v.elemento);
        if (verticeComparar == null || v.getGrado() != verticeComparar.getGrado())
            return false;
        for (Vecino vecino : v.vecinos) {
            boolean encontrada = false;
            for (VerticeGrafica<?> vecinoComparar : verticeComparar.vecinos()) {
                if (vecino.vecino.elemento.equals(vecinoComparar.get()) && vecino.peso == ((Vecino) vecinoComparar).peso) {
                    encontrada = true;
                    break;
                }
            }
